...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
            ...
            set
            {
                if (_hoveringInteractable != value)
                {
                    ...
                    if (_hoveringInteractable != null)
                    {
                        ...
                        //OMA
                        if (_hoveringInteractable.gameObject.tag.Equals("Door"))
                        {
                            HandleDoor(_hoveringInteractable.gameObject);
                        }
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            Debug.Log("tarttuu esineeseen: " + objectToAttach.name);
        ...
        public void DetachObject(GameObject objectToDetach, bool restoreOriginalParent = true)
        {
            ...
            if (index != -1)
            {
                ...
                //Debug.Log("pudottaa esineen: " + objectToDetach.name);

                //GameObject.FindObjectOfType<FileLogger>().printProgress("Pudottaa esineen");
        ...
        //OMA
        public void HandleDoor(GameObject door)
        {           
            Debug.Log("door touched");

            if (!doorIsOpen)
            {
                if (door.gameObject.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("Empty"))
                {                    
            ...
            {
                if (door.gameObject.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("DoorOpenAnimation") 
                    && door.gameObject.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).normalizedTime >= 1)
                {           
                        door.gameObject.GetComponent<Animator>().SetBool("Open", false);
                        doorIsOpen = false;
